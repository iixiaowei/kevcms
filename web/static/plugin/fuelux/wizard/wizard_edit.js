		...
		next: function () {
			var validator = $("#fuelux-wizard").validate({
			    ...
			    errorPlacement: function(error, element) {
					 if (error.html() != "") {
			              ...
			              if( element.attr("name") == "p_serial_sn" || element.attr("name") == "p_name_id" || element.attr("name") == "p_date" || element.attr("name") == "tickets" ){
			...
			if(validator.form()){
				var form = $('#fuelux-wizard');
				 var ajax = {
						url: "/admin/sale/saveedit_one", data: form.serialize(), type: 'POST', dataType: 'json', cache: false,
						success: function(data, statusText) {
							
							if (data.status == 1) {
								
								window.location.href="/admin/sale/edit_two/id/"+data.id;		
								return false;	
							}
